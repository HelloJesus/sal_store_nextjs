...
const Cart = () => {
    ...
    const { data: cart, mutate: cartMutate } = useCart(user)

    const deleteCart = (productId: string) => fetcherParams({
        method: "DELETE",
        url: productId && `${process.env.NEXT_PUBLIC_API_HOST}/cart/` + productId,
        headers: { "Content-type": "application/json" },
    })

    const hadleDeleteCart = async (productId: string) => {
        await cartMutate(deleteCart(productId),
            {
    ...
    return (
        <>
            ...
            <div className="grid grid-cols-1 gap-5 lg:grid-cols-2">
                {Array.isArray(cart) && cart?.map((elem, index): any => {
                    return (
                        <div key={index} className="py-3 px-2.5 bg-slate-200 rounded-lg flex gap-2.5 justify-between items-center flex-col sx:flex-row">
                            ...
                            <div className="flex flex-col gap-2 max-w-[120px] w-full">
                                <button className="rounded-full bg-blue-500 text-white px-2.5 py-1 active:bg-blue-700">
                                    Buy
                                </button>
                                <button className="rounded-full bg-red-500 text-white px-2.5 py-1 active:bg-red-700"
                                    onClick={() => hadleDeleteCart(elem.productId)}>
                                    delete
                                </button>
                            </div>
                        </div>
